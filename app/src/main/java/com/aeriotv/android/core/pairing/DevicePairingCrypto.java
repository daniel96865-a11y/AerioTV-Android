...
/** Authenticated six-digit pairing without transmitting the PIN or an offline verifier. */
final class DevicePairingCrypto {
    static final int VERSION = 0x53335031; // S3P1
    ...
    static final class Session implements AutoCloseable {
        ...
        private Session(byte[] key, byte[] context) {
            this.key = key;
            this.context = context;
        }

        byte[] encrypt(byte[] plaintext) throws Exception {
            if (plaintext.length > MAX_PAYLOAD) throw new IOException("Wiedergabeliste zu groß");
        ...
        byte[] decrypt(byte[] ciphertext) throws Exception {
            if (ciphertext.length < 28 || ciphertext.length > MAX_PAYLOAD + 28) {
                throw new IOException("Ungültige Wiedergabeliste");
            }
    ...
    static Session exchange(DataInputStream in, DataOutputStream out, String pin, boolean host) throws Exception {
        if (pin == null || !pin.matches("[0-9]{6}")) throw new IOException("Ungültiger Code");
        String id = (host ? "host:" : "client:") + UUID.randomUUID();
        char[] password = pin.toCharArray();
        JPAKEParticipant participant;
        try {
            participant = new JPAKEParticipant(id, password);
        } finally {
        ...
        writeNumber(out, first.getGx1());
        writeNumber(out, first.getGx2());
        writeProof(out, first.getKnowledgeProofForX1());
        writeProof(out, first.getKnowledgeProofForX2());
        out.flush();

        if (in.readInt() != VERSION) throw new IOException("Beide Geräte benötigen Streamy 3.0.5 oder neuer");
        ...
        participant.validateRound1PayloadReceived(
                new JPAKERound1Payload(peer, readNumber(in), readNumber(in), readProof(in), readProof(in))
        );

        JPAKERound2Payload second = participant.createRound2PayloadToSend();
        out.writeUTF(id);
        writeNumber(out, second.getA());
        writeProof(out, second.getKnowledgeProofForX2s());
        out.flush();
        participant.validateRound2PayloadReceived(
                new JPAKERound2Payload(readId(in), readNumber(in), readProof(in))
        );
        ...
        out.writeUTF(id);
        writeNumber(out, third.getMacTag());
        out.flush();
        participant.validateRound3PayloadReceived(
                new JPAKERound3Payload(readId(in), readNumber(in)),
                material
        );

        byte[] context = ("Streamy 3 pairing v1\n" +
    ...
    private static void writeNumber(DataOutputStream out, BigInteger number) throws IOException {
        writeBytes(out, number.toByteArray());
    }

    private static BigInteger readNumber(DataInputStream in) throws IOException {
        return new BigInteger(readBytes(in, 385));
    }

    private static void writeProof(DataOutputStream out, BigInteger[] proof) throws IOException {
        writeNumber(out, proof[0]);
        writeNumber(out, proof[1]);
    }

    private static BigInteger[] readProof(DataInputStream in) throws IOException {
        return new BigInteger[]{readNumber(in), readNumber(in)};
    }
